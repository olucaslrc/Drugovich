...
namespace Drugovich.Presentation.Controllers.App
{
    ...
    public class CustomersController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetCustomers()
        {
            try
            {
                return new ObjectResult(await _mediator.Send(new GetCustomersQuery()));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> AddCustomer([FromBody]AddCustomerDTO model)
        {
            try
            {
                return new ObjectResult(await _mediator.Send(new AddCustomerCommand(model)));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex.Message, DateTime.UtcNow);
        ...
        public async Task<IActionResult> UpdateCustomerGroup([FromBody]CustomerDTO model)
        {
            try
            {
                return new ObjectResult(await _mediator.Send(new UpdateCustomerCommand(model)));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex.Message, DateTime.UtcNow);
